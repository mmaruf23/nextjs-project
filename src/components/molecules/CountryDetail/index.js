...
export default function CountryDetail({dataCountry}) {
  return (
    <div className="border border-red-400 rounded-lg w-full p-5 flex flex-col gap-2">
      ...
      <List keyName={'Official Name'} valueName={`${dataCountry?.name.official}`}/>
      <List keyName={'Capital'} valueName={`${dataCountry?.capital}`}/>
      <List keyName={'Languages'} valueName={`${Object.values(dataCountry?.languages || {})}`}/>
      <List keyName={'Currencies'} valueName={`${Object.keys(dataCountry?.currencies || {})}`}/>
